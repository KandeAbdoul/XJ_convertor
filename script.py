    ...
    for nodes in root.iter('complexType') : 
        for child in nodes:
            if child.tag == "sequence": # permet de verifier si c'est une classe
                ...
                classes.append(nodes.attrib['name'])
...
def getHeritEntites(fichier):
    ...
    for nodes in root.iter('complexType') : 
        for child in nodes:
            for nodes in root.iter('complexType') : 
                for child in nodes:
                    child.tag == "complexContent": #verifier si c une classe heritiere
...
def FunctionName(fichier):
    ...
    for nodes in root.iter('complexType') : 
        for child in nodes:
            child.tag == "simpleContent": #verifier si c'est une classe permet de declarer un type de donnees
...
def getRelations(fichier):
    ...
    for nodes in root.iter('complexType') :
        for child in nodes:
            child.tag == "relation": # permet de verifier si c'est une classe
                relations.update({nodes.attrib['name']:{'multiplicite':{}}}) #inserer le nom dans entites en initialisants ses attibuts a vide
                for attribut in child:
                    for seq in attribut:
                        relations[nodes.attrib['name']]['multiplicite'].update({seq.attrib['name']:(seq.attrib['minOccurs'],seq.attrib['maxOccurs'])})
    return relations
